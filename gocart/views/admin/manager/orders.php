...
<div class="row">
	<div class="span12" style="border-bottom:1px solid #f5f5f5;">
		<div class="row">
			...
			<div class="span8">
				<h6><span class="heading-text-3">Search and Export Orders</span></h6>
				<?php echo form_open($this->config->item('admin_folder').'/orders/allOrders', 'class="form-inline" style="float:right"');?>
					<fieldset>
						...
						<a class="btn btn-info" style="float:right;" href="<?php echo site_url('admin/orders/allOrders'); ?>">RESET </a>
					</fieldset>
				</form>
			</div>
		</div>
...
<table class="table table-striped">
    <thead>
		<tr>
			<th>
				<input type="checkbox" id="gc_check_all" style="float:left;margin:8px 0px 0px 0px;" />
				<button type="submit" class="btn btn-small btn-danger" style="float:left;margin:0px 0px 0px 5px;"><i class="icon-trash icon-white"></i></button>
				<select id="allstatus" class="span2" style="float:left;margin:0px 0px 0px 32px;">
				...
				<button type="button" class="btn" style="float:left;margin:0px 0px 0px 5px;" onclick="updatemultipleorderstatus()">Save</button>
			</th>
	...
	<?php if($order->status=='Order Placed' || $order->status=='Cancelled'){ ?>
	<tr>
		...
		<td style="white-space:nowrap"><?php echo date('m/d/y', strtotime($order->ordered_on)); ?></td>
		<td style="span2"> 
			<select name="status" id="status_form_<?php echo $order->id; ?>" class="span2" style="float:left;">
				<?php if($AdminAccess=='Orders'){ ?>
	...
	<tr>
		...
		<td style="white-space:nowrap"><?php echo $order->ship_lastname.', '.$order->ship_firstname; ?></td>
		<td style="white-space:nowrap"><?php echo date('m/d/y', strtotime($order->ordered_on)); ?></td>
		<td style="span2"><?php echo $order->status; ?></td>
		<td><div class="MainTableNotes"><?php echo format_currency($order->total); ?></div></td>
		<td>
			<a class="btn btn-small" style="float:right;"href="<?php echo site_url($this->config->item('admin_folder').'/orders/orderDetail/'.$order->id.'?AccessToken=kasdfji329sdfakafsdfks42');?>"><i class="icon-search"></i> <?php echo lang('form_view')?></a>
		</td>
	</tr>
	<?php } ?>
    <?php endforeach; ?>
...
function updatemultipleorderstatus(){
	...
	if(ids!='' && status!=''){
		show_animation();
		$.post("<?php echo site_url($this->config->item('admin_folder').'/orders/updatemultipleorderstatus'); ?>", {ids:ids,status:status}, function(data){
			hide_animation();
		});
	}
}
