...
<form action="<?php echo site_url().$this->config->item('admin_folder').'/products/form/'.$id; ?>" method="post" enctype="multipart/form-data" accept-charset="utf-8">
<div class="row">
	<div class="span8">
		<div class="tabbable">
			<ul class="nav nav-tabs">
				...
				<li><a href="#additional_options" data-toggle="tab"><?php echo lang('additional_options');?></a></li>
				<li><a href="#pr_downloads" data-toggle="tab">Pictures</a></li>
		...
		<div class="tab-content">
			...
			<div class="tab-pane" id="product_related">
				...
				<div class="row">
					<div class="span2" style="text-align:center">
						...
						<div class="row">
							<div class="span5">
								<select class="span5" id="product_list" size="5" style="margin:0px;"></select>
							</div>
						</div>
						<div class="row">
			...
			<div class="tab-pane" id="additional_options">
				<div class="row">
					<div class="span8">
						<label>Specification</label>
						<?php
						$data	= array('name'=>'specifications', 'class'=>'redactor', 'value'=>set_value('specifications', $specifications));
						echo form_textarea($data);
						?>
				...
				<div class="row">
					<div class="span8">
						<label>FAQs</label>
						<?php
						$data	= array('name'=>'faqs', 'class'=>'redactor', 'value'=>set_value('faqs', $faqs));
						echo form_textarea($data);
						?>
				...
				<div class="row" style="padding-top:10px;">
								<div class="span3">
									<label for="qualifier"><?php echo lang('qualifier');?> </label>
									<?php
								 	$options = array(	 'EA'	=> 'EA',
														'package'	=> 'package',
														'CS 12'	=> 'CS 12',
									...
									echo form_dropdown('qualifier', $options, set_value('qualifier',$qualifier), 'class="span3"');
									?>
								</div> 
								<div class="span3">
									<label for="track_stock"><?php echo lang('oversized');?> </label>
									<?php
								 	$options = array(	 'Yes'	=> 'Yes'
														,'No'	=> 'No'
														);
									echo form_dropdown('oversized', $options, set_value('oversized',$oversized), 'class="span3"');
									?>
								</div> 
				</div>
			...
			<div class="tab-pane" id="pr_downloads">
				<div class="row">
					<div class="span8">
						<label>DOWNLOADS <a onclick="addMorePDFs()" class="btn btn-success" style="float:right; font-size:9px;margin-right:5px;"> ADD MORE</a></label> 
						<table id="pdffiletableid" cellpadding="10" cellspacing="10" width="100%">
							<tr>
								<th>Title</th>
								<th>FileName</th>
								<th>Attach</th>
							...
							<?php $pdfcounter = 1; ?>
							<?php foreach($productfiles as $pdffiles){ ?>
							<tr id="pdffile_row_<?php echo $pdfcounter; ?>" class="pdfrows">
								<td><input class="span2" name="pdftitle_old[]" value="<?php echo $pdffiles->title; ?>" type="text"></td>
								<td><input class="span2" name="pdffile_name_old[]" value="<?php echo $pdffiles->file_name; ?>" type="text" readonly="readonly"></td>
								<td><input type="hidden" name="pdffile_path_old[]" value="<?php echo $pdffiles->file_path; ?>" /></td>
								<td><a onclick="removePDF(<?php echo $pdfcounter; ?>)" class="btn btn-danger" style="float:right; font-size:9px;"><i class="icon-trash icon-white"></i> Remove</a></td>
							</tr>
							<?php $pdfcounter++; } ?>
							<?php }else{ ?>
							<tr id="pdffile_row_1" class="pdfrows">
								<td><input class="span2" name="pdftitle_new[]" value="" type="text"></td>
								<td>&nbsp;</td>
								<td><input type="file" name="pdffile_new[]" /></td>
								<td><a onclick="removePDF(1)" class="btn btn-danger" style="float:right; font-size:9px;"><i class="icon-trash icon-white"></i> Remove</a></td>
							</tr>
							<?php } ?>
						</table>
					</div>
            ...
            <div class="tab-pane" id="return_policy">
				<div class="row">
					<div class="span8">
						<label>Enter Return Policy </label> 
						<div class="row">
					<div class="span8">
						
						<?php
						$data	= array('name'=>'return_policy', 'class'=>'redactor', 'value'=>set_value('return_policy', $return_policy));
						echo form_textarea($data);
						?>
	...
	<div class="span4">
        ...
        <select class="span4 selectpicker" name="publisherId" data-live-search="true" style="width:100%">
        <option>Select</option>
        <?php if(count($publisher_option)>0){ ?>
        <?php foreach($publisher_option as $publisher){ ?>
        <option value="<?php echo $publisher->publisherId; ?>" <?php if($publisherId==$publisher->publisherId){echo 'selected="selected"';} ?>><?php echo $publisher->publisherName; ?></option>
        <?php }} ?>
        </select>
        
        <label>Select Author</label>
        <select class="span4 selectpicker" name="authorId" data-live-search="true">
        <option>Select</option>
        <?php if(count($author_option)>0){ ?>
        <?php foreach($author_option as $author){ ?>
        <option value="<?php echo $author->authorId; ?>" <?php if($authorId==$author->authorId){echo 'selected="selected"';} ?>><?php echo $author->authorName; ?></option>
        <?php }} ?>
        </select>
        ...
        <select class="span4 selectpicker" name="editionId" data-live-search="true">
        <option>Select</option>
        <?php if(count($edition_option)>0){ ?>
        <?php foreach($edition_option as $edition){ ?>
        <option value="<?php echo $edition->editionId; ?>" <?php if($editionId==$edition->editionId){echo 'selected="selected"';} ?>><?php echo $edition->editionName; ?></option>
        <?php }} ?>
        </select>
		<br /><br />
		<?php
		...
		<label for="saleprice"><?php echo "Sale / Discounted Price"; //lang('saleprice');?></label>
		<?php
		$data	= array('name'=>'saleprice', 'value'=>set_value('saleprice', $saleprice), 'class'=>'span4');
		echo form_input($data);?>
		<label>Select Currency </label>
        <select class="span4" name="currency_symbol">
        <option>Select</option>
		<?php 
		if(count($currency)>0){ ?>
        <?php foreach($currency as $cur){ ?>
        <option value="<?php echo $cur->symbol; ?>" <?php if($cur->symbol==$currency_symbol){echo 'selected="selected"';} ?>><?php echo $cur->symbol; ?></option>
        <?php }} ?>
        </select>
		
		<label for="saleprice_start_date">Sale Price Start Date</label>
		<?php
		$data	= array('name'=>'saleprice_start_date','value'=>set_value('saleprice_start_date',$saleprice_start_date),'class'=>'span4','type'=>'date');
		echo form_input($data);?>
		
		<label for="saleprice_start_date">Sale Price End Date</label>
		<?php
		$data	= array('name'=>'saleprice_end_date','value'=>set_value('saleprice_end_date',$saleprice_end_date),'class'=>'span4','type'=>'date');
		echo form_input($data);?>		
	</div>
</div>
...
<div class="form-actions">
    ...
    <a href="<?php echo $_SERVER['HTTP_REFERER']; ?>" class="btn btn-success">Back</a>
</div>
</form>
...
<script type="text/javascript">
function removePDF(pdfrowid){
	$("#pdffile_row_"+pdfrowid).remove();
}
function addMorePDFs(){
	var pdfrowslen = $(".pdfrows").length;
	pdfrowslen = pdfrowslen++;
	var html = '<tr id="pdffile_row_'+pdfrowslen+'" class="pdfrows"><td><input class="span2" name="pdftitle_new[]" value="" type="text"></td><td>&nbsp;</td><td><input type="file" name="pdffile_new[]" /></td><td><a onclick="removePDF('+pdfrowslen+')" class="btn btn-danger" style="float:right; font-size:9px;"><i class="icon-trash icon-white"></i> Remove</a></td></tr>'; 
	$("#pdffiletableid").append(html);
}
//<![CDATA[
