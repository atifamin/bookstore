...
class Cart extends Front_Controller {
	...
	function category($id)
	{
		...
		$data['subcategories'] = $this->Category_model->get_categories($id);
		...
		$data['products']	= $this->Product_model->get_products($data['category']->id, $config['per_page'], $page, 'sku', "ASC");
		...
		$this->view('category', $data);
		//$this->view('category_product', $data);
	}
	...
	{ 
		...
		if( ! $status['validated'])
		{
			...
			// Bilal Code
			$data['product']	= $this->Product_model->get_product($this->input->post('id'));
			echo '<div class="coffee-span-12">
				  <div class="container container-2b" style="width:100%;float:left;">
					<div class="coffee-span-8"><a href="'.site_url().'/'.$data['product']->slug.'"><h6 class="heading-6"><span class="heading-text-5">'.$data['product']->name.'</span><span class="heading-text-5" style="color:#000;"> successfully added in CART.</span></h6></a></div>
					<div class="coffee-span-4"><a class="link-button button-link-1" role="button" href="'.site_url('cart/view_cart').'">CHECKOUT</a></div>
				  </div>
				</div>';
		}
	}
