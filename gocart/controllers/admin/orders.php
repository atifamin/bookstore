...
class Orders extends Admin_Controller {	
	...
	function updateOrderItems($OrderID){
		...
		foreach($_POST['productsid'] as $key=>$val){
			...
			$data['price'] = $productPriceFromOrder;
			$data['saleprice'] = $productPriceFromOrder;
			...
			if(!empty($_POST['options'][$GetPrR->id])){
				$data['options'] = $_POST['options'][$GetPrR->id];
			}else{
				$data['options'] = '';
			}
			$data['is_gc'] = '';
			...
			//echo '<pre>'; print_r($Serialized); exit;
			$this->load->model('customer_model');
			$OI['order_id'] = $OrderID;
	...
	{   
		...
		if ($this->form_validation->run() == TRUE)
		{
			...
			if($orderStatus=='Approved' || $orderStatus=='Order Completed' || $orderStatus=='Cancelled'){
