...
class Checkout extends Front_Controller {
	...
	function step_1()
	{
		...
		$data['cfirstname'] = $data['customer']['firstname'];
		$data['clastname']  = $data['customer']['lastname'];
		
		// Check Temporary Default Address if available
		$data['DefaultAddress'] = $this->customer_model->getTempDefaultAddress($CustomerID);
		if(empty($data['DefaultAddress'])){
			$data['DefaultAddress'] = $this->customer_model->getDefaultAddress($CustomerID);  
		}
		//echo '<pre>';print_r($data['DefaultAddress']);exit;
		
		$data['getAllStates'] = $this->customer_model->listingAllWhere('country_id',223,'gc_country_zones');
	...
	*/
	...
	{		
		...
		$customer = $this->go_cart->customer(); 
		$this->customer_model->removeTempdefaultaddress($customer['id']);
