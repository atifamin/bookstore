    ...
    */
    ...
    function reset_password($email)
    {
        ...
        if ($customer)
        {
			...
			$customer['password_no_hash']   = $new_password;
            $this->save($customer);
	...
	public function listingAllWhere($WhereColumn,$WhereValue,$TableName){
		$this->db->select("*");
	...
	public function listingSingleWhere($WhereColumn,$WhereValue,$TableName){
		$this->db->select("*");
		$this->db->from("$TableName");
		$this->db->where(''.$WhereColumn.' = '.$WhereValue.'');
		$Query = $this->db->get();
	...
	public function setasdefaultaddress($CustomerID,$AddressID){
		$this->db->set('default', 'N');  
		$this->db->where('customer_id', $CustomerID); 
		$this->db->update('gc_customer_addresses');
		
		$this->db->set('default', 'Y');  
		$this->db->where('id', $AddressID); 
		$this->db->update('gc_customer_addresses');
	}
	public function setasTempdefaultaddress($CustomerID, $AddressID){
		$this->db->set('tmp_default', 'N');  
		$this->db->where('customer_id', $CustomerID); 
		$this->db->update('gc_customer_addresses');
		
		$this->db->set('tmp_default', 'Y');  
		$this->db->where('id', $AddressID); 
		$this->db->update('gc_customer_addresses');
	}
	public function removeTempdefaultaddress($CustomerID){
		$this->db->set('tmp_default', 'N');  
		$this->db->where('customer_id', $CustomerID); 
		$this->db->update('gc_customer_addresses'); 
	}
	public function deleteaddress($AddressID){
		$this->db->where('id', $AddressID); 
		$this->db->delete('gc_customer_addresses');
	}
	public function deletefavourite($ProductID,$CustomerID){
		$this->db->where('customer_id', $CustomerID);
		$this->db->where('product_id', $ProductID); 		
		$this->db->delete('gc_favourites');
	}
	public function getFavouriteProducts($CustomerID){
		$this->db->select("gcp.id as productid, gcp.name, gcp.sku, gcp.slug, gcp.route_id, gcp.price, gcp.saleprice, gcp.images, gcp.enabled");
		$this->db->from("gc_favourites gcf");
		$this->db->join("gc_products gcp","gcp.id = gcf.product_id"); 
		$this->db->where('gcf.customer_id = '.$CustomerID.'');
		$this->db->order_by("gcf.created_at DESC");
		$this->db->limit(100); 
		$Query = $this->db->get(); 
		return $Query->result();
	}
	public function getAddressDetail($AddressID){
		$this->db->select("*");
		$this->db->from('gc_customer_addresses');
		$this->db->where('id',$AddressID); 
		$Query = $this->db->get();
		return $Query->row();
	}
	...
	public function getTempDefaultAddress($CustomerID){
		$this->db->select("*");
		$this->db->from('gc_customer_addresses');
		$this->db->where('customer_id',$CustomerID); 
		$this->db->where('tmp_default','Y'); 
		$Query = $this->db->get();
		return $Query->row();
	}
	public function getCustomerAddresses($CustomerID){
		$this->db->select("gca.*, gcza.code");
		$this->db->from("gc_customer_addresses gca");
		$this->db->join("gc_country_zones gcza","gcza.id = gca.state"); 
		$this->db->where('gca.customer_id = '.$CustomerID.''); 
