...
Class Product_model extends CI_Model
{
	...
	{
		return	$this->db->like('name', $name)->or_like('sku',$name)->get('products', $limit)->result();
	}
		...
		else
		{
			...
			if(!empty($data['term']))
			{
				...
				if(!empty($search->created_at)){
					$daterange = explode(" - ", $search->created_at);
	...
	function get_product_files($id){
		$this->db->select('*');
		$this->db->from('gc_products_medias');
		$this->db->where('product_id', $id);
		$Query = $this->db->get(); 
		return $Query->result();
	}
	function deleteProductFiles($id){
		$this->db->where('product_id', $id);
		$this->db->delete('products_medias');
	}
	public function addPDFs($data){
		$this->db->insert('products_medias', $data);
