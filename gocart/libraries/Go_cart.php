...
class go_cart {
		...
		if($recalculate) 
		{
			...
			foreach ($this->_cart_contents['items'] as $key => &$val)
			{
				...
				if ( $val['shippable']== 1 )
				{
					...
					if(is_numeric($val['weight']) && is_numeric($val['quantity'])){
	...
	********************************************************/
	...
	function save_order() {
		...
		$AddInfo = $this->CI->session->userdata("AdditionalInformation"); 
		$save['po_number']					= $AddInfo['po_number'];
		$save['additional_instructions']	= $AddInfo['additional_instructions'];
